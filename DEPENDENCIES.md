...
| `springdoc-openapi-starter-webmvc-ui` | Automatic API documentation: generates OpenAPI 3 spec and serves Swagger UI. | `controller/*Controller.java` (`@Tag`, `@Operation`), `dto/*` (`@Schema`). Endpoints: `/swagger-ui.html`, `/v3/api-docs`. |
...
- Entity lifecycle: `@Entity`, `@Id`, `@Column`, `@Table`, `@ManyToOne`, `@OneToMany`.
...
- `@Min` / `@DecimalMin`, prevents values below minimum.
